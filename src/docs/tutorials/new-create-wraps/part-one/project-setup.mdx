...
import ProjectDirCommon from '../components/_project-dir-common.mdx';
...
We will generate our project scaffolding using the Polywrap CLI. The `create` command of the Polywrap CLI uses the syntax `polywrap create <project type> <language> <name>`. This tutorial is about building Wasm wraps, so let's create a `wasm` project and name it `oracle-wrap`
